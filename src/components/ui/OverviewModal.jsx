  ...
  return createPortal(
    ...
    >
      <div
        ...
        className="w-full max-w-xl animate-pop-in rounded-2xl border border-white/10 bg-ink-700 p-6 shadow-2xl sm:p-7"
      >
        <div className="mb-4 flex items-start justify-between gap-4">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="grid h-9 w-9 shrink-0 place-items-center rounded-lg text-slate-400 transition-colors hover:bg-white/5 hover:text-white"
          >
            <X size={18} />
          </button>
        </div>

        <p className="max-h-[60vh] overflow-y-auto whitespace-pre-wrap text-sm leading-relaxed text-slate-300">
          {text}
        </p>
      </div>
    </div>,
